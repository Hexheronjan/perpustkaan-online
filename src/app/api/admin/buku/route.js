...
// VERSION: Auto-approve for Admin
...
async function getUserFromRequest(req) {
  try {
    ...
    if (req?.cookies && typeof req.cookies.get === 'function') {
      token = req.cookies.get('token')?.value || req.cookies.get('session')?.value;
    }
    if (!token) {
      const cookieStore = await cookies();
      token = cookieStore.get('token')?.value || cookieStore.get('session')?.value;
    }
  ...
  try {
    ...
    const createdId = await withTransaction(async (client) => {
      ...
      `, [
        ...
        'approved', userId, userId, new Date()
      ]);
    ...
    console.log('✅ Buku created (Auto-Approved):', result.rows[0].judul);
    return NextResponse.json(result.rows[0], { status: 201 });
  } catch (error) {
    console.error('❌ Error creating buku:', error);
    return NextResponse.json({ message: 'Gagal menambah buku', error: error.message }, { status: 500 });
  }
}
...
export async function PUT(req) {
  ...
  try {
    ...
    await withTransaction(async (client) => {
      ...
      if (checkResult.rows.length === 0) throw new Error('Buku tidak ditemukan');
      ...
      Object.keys(updateFields).forEach(key => {
        if (updateFields[key] !== undefined && key !== 'id' && key !== 'is_approved' && key !== 'updated_at' && key !== 'status' && key !== 'approved_by' && key !== 'approved_at') {
          paramCount++;
    ...
    console.log('✅ Buku updated (Auto-Approved):', result.rows[0].judul);
    return NextResponse.json(result.rows[0]);
  } catch (error) {
    console.error('❌ Error updating buku:', error);
    return NextResponse.json({ message: 'Gagal update buku', error: error.message }, { status: 500 });
  }
}
...
export async function DELETE(req) {
  ...
  try {
    ...
    return NextResponse.json({ message: 'Gagal hapus buku', error: error.message }, { status: 500 });
  }
}