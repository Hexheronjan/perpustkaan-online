...
export async function GET(req) {
	...
	if (!authCheck.ok) {
		...
		let debugInfo = "No auth header";
		if (authHeader) {
			const token = authHeader.split(' ')[1];
			try {
				const decoded = verifyJwt(token);
				debugInfo = decoded ? `Valid Token. RoleID: ${decoded.role_id}` : "Invalid Token (verifyJwt returned null)";
			} catch (e) {
				debugInfo = `Verify Error: ${e.message}`;
			}
		}

		return NextResponse.json({
			success: false,
			message: `Akses ditolak! Role Anda: ${authCheck.role}. Butuh: staf atau admin. Debug: ${debugInfo}`,
