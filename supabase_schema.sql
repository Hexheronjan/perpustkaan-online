-- ============================================================
-- SUPABASE SCHEMA - Perpustakaan Online
...
CREATE INDEX IF NOT EXISTS idx_buku_genre ON buku(genre_id);
...
CREATE INDEX IF NOT EXISTS idx_peminjaman_buku ON peminjaman(buku_id);
CREATE INDEX IF NOT EXISTS idx_peminjaman_status ON peminjaman(status);
CREATE INDEX IF NOT EXISTS idx_buku_pending_status ON buku_pending(status);
...
DROP TRIGGER IF EXISTS update_buku_updated_at ON buku;
...
DROP TRIGGER IF EXISTS update_buku_pending_updated_at ON buku_pending;
...
CREATE OR REPLACE VIEW view_buku_lengkap AS
SELECT 
    b.id,
    b.judul,
    b.penulis,
...
-- ============================================================
-- CATATAN PENTING:
-- Setelah menjalankan schema ini, buat akun admin pertama
-- dengan menjalankan query berikut (ganti password hash):
-- 
-- INSERT INTO users (username, email, password, nama_lengkap, role_id)
-- VALUES ('admin', 'admin@perpustakaan.com', '$2a$10$HASH_BCRYPT_DISINI', 'Administrator', 4);
--
-- Gunakan script scripts/create-admin.js atau buat hash bcrypt
-- dari password yang Anda inginkan.
-- ============================================================
